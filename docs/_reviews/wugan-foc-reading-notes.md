---
title: "《无感FOC入门指南》阅读笔记"
date: 2024-02-05
type: book
cover_label: "FOC 电机控制"
cover_color: "linear-gradient(135deg,#006064,#00838f)"
brief: "阳波《无感FOC入门指南》和上官致远《深入理解无刷直流电机矢量控制技术》两本FOC书籍的系统性阅读笔记，涵盖坐标变换、SVPWM、PI调节、无感位置估计等核心内容。"
---
...
![矢量表](/assets/images/book1_form_4-1.jpeg)
<p style="text-align:center; color:orange">图1：矢量表</p>

![扇区图](/assets/images/book1_img_4-25.jpeg)
...
![矢量圆半径](/assets/images/book1_img_4-38.jpeg)
...
![中性点电压图](/assets/images/book1_img_4-26.jpeg)
...
![扇区2占空比计算图](/assets/images/book1_img_4-40.jpeg)
...
在计算占空比之前，最好先确保${V_\alpha}^2 + {V_\beta}^2 \leq V_R^2$，即：

$$ {V_\alpha}^2 + {V_\beta}^2 \leq \frac{V_M^2}{3}  $$ 

否则会出现超出矢量圆范围的矢量。如果超过了，就需要对$V_\alpha$和$V_\beta$进行等比例缩小以确保其处于矢量圆范围内。
...
![锁相环原理图之1](/assets/images/LPL_arch_1.jpeg)
<p style="text-align:center; color:orange">图1：锁相环控制原理</p>

![锁相环原理图之1](/assets/images/LPL_detail_2.jpeg)
...
![直流电机换相](/assets/images/bdc_motor_demo.png)
...
#### 2.3.3 PMSM电机的几个坐标方程
![三相绕组](/assets/images/permmag_syncmotor_windings.png)
...
![矢量控制架构图](/assets/images/FOC_Arch_310.jpeg)
...
![电流换PI控制参数估算](/assets/images/Current_Loop_Parameters.jpeg)
...
#### 2.4.3 矢量作用时间计算
这一部分本书的推导公式相当繁琐，没有抓住问题的本质。建议参考第一本书中相关内容。（第一本书公式稍有问题，不知道是编写问题还是印刷问题。而这本书开头讲的还可以，后面有点混水摸鱼的感觉。）

这本书给出的不是直接的占空比，但是根据公式也可以比较简单的计算出占空比。

需要说明的是本书30页提到的正旋定理证明有两种做法，一种可以用边长和垂线之积是三角性面积的两倍然后用多种方式表示面积，组成一个方程式就可以证明边长除以它对角的正旋值是个固定值；另一种方法可以做一个辅助线，找到到三个顶点距离相等的一点（三角形外接圆圆形），然后可以很轻松的证明某个边和连接这个边顶点的半径组成的三角形圆心角是对角的倍数的2倍。做圆形角的二分线到边上，可证明这条二分线和边长垂直。这样正可以很轻松的证明外接圆的半径等于边长除以对角的正旋值。

![正旋定理证明辅助图](/assets/images/Proof_of_sine_theorem.jpg)

<p style="text-align:center; color:orange">图?：正旋定理证明辅助图</p>


#### 2.4.4 七段式调制方式分析
这一部分对与七段式和五段式的描述似乎有些问题。在本书的描述中似乎只有七段式是中心对称式，而五段式不是。实际上五段式也是中心对其方式。不知道是我本人的理解错误，还是？但有一点是确定的，五段式谐波分量比七段式的谐波分量多。在搜索网络时也看到有人提出过类似的说法。具体原因说是因为少了一段零向量。我个人暂时不太理解这个理由到底是怎么推断出来的。为什么少一段零向量就导致谐波分量多了？后面再慢慢研究吧，道阻且长！

5段式的，拿第1扇区来说变化过程是000->100->110->100->000，而7段式的变化过程是000->100->110->111->101->000。这两种每次都是只改变一相功率管的状态，所以不太确定原因。

这部分描述了七段式调制扇区变化的具体情况。波形和第一本书一致。不再赘述。

#### 2.4.5 PWM占空比计算
其实这一部分可以根据2.4.3推导出来。只是要注意七段式中$V_0$和$V_7$平均分配剩余的时间。还需要注意一段对称模式的PWM周期是我们求得的周期的一半。（这是对称式PWM的特性）

在第一本书中计算的是占空比，这本书计算的是周期。现在的单片机PWM设置一般是设置周期和激活周期，所以本书的计算方法可能更合适。但我知道也有个别厂家的MCU需要设置的是占空比，所以还是有必要了解一下。（具体那家我忘了）

本书还强调了死区时间的重要性。提到死区时间补偿一致是电机控制首要解决的问题。
#### 2.4.6 死区和死区补偿
为了防止上下管同时导通，经常要引入死区补偿。（MOSFET的关断时长比开启时长经常要更长一些，不同的MOSFET这个参数也不一样。）但是死区补偿又会导致波形畸变
...
![弱磁控制示意](/assets/images/mathworks_fwc_control_intro.png)
...
![直接转矩控制示意](/assets/images/mathworks_dtc_algorithm.png)
